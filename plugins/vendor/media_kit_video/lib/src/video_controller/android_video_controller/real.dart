  ...
  AndroidVideoController._(super.player, super.configuration) {
    ...
    videoParamsSubscription = player.stream.videoParams.listen(
      (event) => lock.synchronized(() async {
        final rawWidth = event.w ?? event.dw ?? 0;
        final rawHeight = event.h ?? event.dh ?? 0;
        final currentWidth = rect.value?.width.toInt();
        final currentHeight = rect.value?.height.toInt();
        final hasDisplayGeometry = event.dw != null && event.dh != null;
        final hasExplicitRotation = event.rotate != null;
        final isBootstrapProbeOnly =
            !hasExplicitRotation &&
            !hasDisplayGeometry &&
            currentWidth == null &&
            currentHeight == null;
        final shouldKeepCurrentSurfaceSize =
            !hasExplicitRotation &&
            !hasDisplayGeometry &&
            currentWidth != null &&
            currentHeight != null &&
            currentWidth > 0 &&
            currentHeight > 0;
        if (isBootstrapProbeOnly) {
          return;
        }
        final rotate = event.rotate ?? 0;
        final isRotated = rotate % 180 != 0;
        final displayWidth = event.dw ?? rawWidth;
        final displayHeight = event.dh ?? rawHeight;
        final width = shouldKeepCurrentSurfaceSize
            ? currentWidth
            : (isRotated ? rawHeight : displayWidth);
        final height = shouldKeepCurrentSurfaceSize
            ? currentHeight
            : (isRotated ? rawWidth : displayHeight);
